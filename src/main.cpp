...
#if defined(NDEBUG)
# error "IlluminatiCoin cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // IlluminatiCoin: starting difficulty is 1 / 2^12
...
{
    // IlluminatiCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // IlluminatiCoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // IlluminatiCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // IlluminatiCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // IlluminatiCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // IlluminatiCoin: increase each by adding 2 to bitcoin's value.
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    RenameThread("illuminaticoin-miner");
